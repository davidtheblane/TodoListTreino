<script>
  ...
  onMount(() => {
    //função Lifecycle: acionada no inicio do componente.
    getTasks();
  });

  function getTasks() {
    fetch("https://todolist-77295.firebaseio.com/tasks.json")
      .then(response => {
        if (!response.ok) {
  ...
  function updateTask() {
    ...
    fetch(`https://todolist-77295.firebaseio.com/tasks/${id}.json`, {
      method: "PATCH",
      body: JSON.stringify(taskData),
      headers: { "Content-Type": "application/json" }
    })
